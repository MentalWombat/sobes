...
export default function ThemeSwitch({ theme }: { theme: Mode | null }) {
  const [mode, setMode] = useState<Mode | null>(theme);

  const update = (state: Mode) => {
    try {
      if (state === 'system') localStorage.removeItem('theme');
      else localStorage.theme = state;
  ...
  return (
    ...
    >
      <span
        ...
        data-checked={mode === 'system' || undefined}
      ...
      <span
        ...
        data-checked={mode === 'light' || undefined}
      ...
      <span
        ...
        data-checked={mode === 'dark' || undefined}
